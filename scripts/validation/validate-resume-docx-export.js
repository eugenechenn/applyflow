...
const { exportTailoredResumeDocx } = require("../../src/lib/resume/resume-exporter");
const {
  createExportDto,
...
async function run() {
  const fixture = readFixture("resume-docx-export-fixture.json");
  const valid = fixture.valid || {};

  const exportDto = createExportDto(valid.exportDto || {});
  const artifact = await exportTailoredResumeDocx(exportDto);
  assertTrue(Buffer.isBuffer(artifact.buffer), "docx artifact buffer must be a Buffer");
  assertTrue(artifact.buffer.length > 0, "docx artifact buffer must not be empty");
  assertTrue(String(artifact.fileName || "").toLowerCase().endsWith(".docx"), "artifact fileName must end with .docx");
  assertTrue(
    artifact.contentType === "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    "artifact contentType must be docx mime type"
  );
  ...
  await expectErrorCode("invalid.exportDto", fixture.invalid?.expectedErrorCode || "INVALID_EXPORT_DTO", async () => {
    await exportTailoredResumeDocx(fixture.invalid?.exportDto || {});
  });
  ...
  console.log("validate-resume-docx-export: docx exporter consumes ExportDTO and contract status lifecycle is consistent.");
}
