...
/**
 * 导出 Batch1 Role Evidence 验证结果：
 * - 6 个对抗 acceptance case（每个 top10）
 * - 6 个 synthetic case
 * 输出到 tmp/role_evidence_batch1.json
 */
...
const OUTPUT_PATH = path.resolve(__dirname, "../../tmp/role_evidence_batch1.json");
...
function summarizeRow(jobVm = {}, index = 0) {
  ...
  return {
    ...
    likelyBundledJD: Boolean(scoring?.jobFeaturesView?.likelyBundledJD),
    jdBlockStructureType: String(scoring?.jobFeaturesView?.jdBlockStructureType || ""),
    ...
    evidencePassThroughOk: Boolean(
      String(evidenceType).trim() &&
      String(roleFitDetails.evidenceType || "").trim()
    ),
    notes: []
  };
}
...
main().catch((error) => {
  console.error("export-role-evidence-batch1 failed:", error?.message || error);
  process.exitCode = 1;
});
