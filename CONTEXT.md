...
当前目标：完成 Internal Beta demo 演示数据池增强（curated 30-40）并保持 real-pool / A-B 内测隔离稳定。
...
- staging 当前 `demo_user=12` 仍是历史 seed 快照；若需线上面试环境生效，需要 staging deploy 后再执行受控 demo reset/reseed（`/api/demo/reset` 目前仍为 501）。
- 4 组画像 Top100 验收：
  - 产品经理+上海：A10/B90
  - 数据分析+上海：A14/B86
  - 算法工程师+上海：A48/B52
...
- 当前可用于“面试演示（本地 curated 复现）+ 本人 staging 自测 + 2-user 白名单真实体验内测”；仍不声明 3-5 人 fully ready / 50-user beta ready / Production Auth Ready。

下一步：
- 若要让线上 demo 使用 curated pool：先走 staging deploy + demo seed/reseed 受控流程，再补远端 smoke 证据。
- 若扩展到 3-5 人内测：先完成 `user_a fallback / payload.userId` 专项治理。
