...
- 2026-06-03：补充异常排查与工程底线面试专题，覆盖静默故障、CDE、SSOT、Human-in-the-loop、回归门禁、北极星指标和 0-1 Agent 工程防线。
...
- 2026-06-03：将正式展示账号调整为无硬地点约束的产品/AI产品画像，新增线上 Top10 全 A、Top50 全 A/B 门禁，并给 D1 候选粗排补充行业偏好信号。
- 2026-06-03：在 Evidence Log 中补充 L3 全量回归预测结果、输出文件格式和面试引用口径，明确不作为已完成数据。
- 2026-06-03：新增作品集专用 Playwright 截图脚本，生成 4 张飞书可读短截图并替换 Evidence Log 推荐截图口径。
- 2026-06-03：补充 Agent 性能评测 60 秒面试口播，并将 Evidence Log 调整为业务转化与 Agent 评估双轨叙事，新增城市 bad case 产品化容错和成本/延迟观测口径。
- 2026-06-03：新增 AI 成本/延迟测量脚本与面试口径，输出 5355 岗位池下 Top10 强模型预算、0.19% 调用比例和本地 token 粗估，并同步作品集 Evidence Log。
- 2026-06-03：将成本/延迟与强模型核心节点调用纳入 00/01/02 高频题入口和 Evidence Log Q&A，补充“不是所有节点都用强模型”和“本地延迟高怎么解释”回答。
- 2026-06-03：补充“强模型只打 Top10 是否仍算 Agent”和“截图/表格数据怎么来的”防守口径，区分小样本漏斗、1000 样本评估、成本画像和线上截图 smoke。
- 2026-06-04：将 02 面试文档全面重构为 9 维度题组作战手册，集中承载 AI 岗位面试口播、项目证据和防守口径。
- 2026-06-04：按用户反馈将 02 再调整为一体化回答结构，项目相关内容直接揉进口播，并加深 Agent 评测、RAG/Memory、成本延迟和异常排查答案。
- 2026-06-04：继续清理 02 中无关硬接 ApplyFlow 的题目，Memory、MCP、多 Agent、通用 Tool Calling 和 ReAct/Plan 改为只讲知识点与风险边界。
- 2026-06-04：为 02 每个题组补充“追问变体怎么答”，让覆盖问法里的 JSON Schema、工具重试、MCP 安全、Memory 污染等具体问题都有可调整回答。
- 2026-06-04：修正 02 中分类任务口播，将 ApplyFlow 岗位数据源分类改写为先建标签体系、再用规则/元数据和 few-shot 处理冷启动与歧义样本。
- 2026-06-04：整合 interview 目录旧标准回答与简历 PDF 事实，补强 02 的 Eval case、网易实习、非科班定位和简历/作品集逐字深挖，并导出 v5 Word。
- 2026-06-04：继续整合面试控场、AI 应用高频考点与 AI 架构材料，给 02 新增题目路由、控场四步法、AI 应用岗四能力和通用场景题五步框架，并导出 v6 Word。
- 2026-06-04：按用户澄清修正 02 结构，删除“7 类 x 4 能力”大矩阵，改为“7 类题型跳转 9 章”的短导航，并导出 v8 Word。
- 2026-06-04：按用户新增视频总结升级 02 的 RAG 技术选型和工程级 Prompt 两个高频大题，并导出 v9 Word。
- 2026-06-04：整理 GitHub 展示仓库，改写面试官版 README，并从 Git 跟踪中移除 node_modules、SQLite、Word 导出和 interview 本地复习资料。
